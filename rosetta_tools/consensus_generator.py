...
CONCEPTS = {
    ...
    # === Security-domain concepts (Vector Bootcamp — Concept Integrity Auditor) ===
    "authorization": {
        "description": "Whether a text describes access or action that is explicitly authorized, permissioned, and within policy vs unauthorized, bypassing controls, or acting outside granted permissions",
        "pos_label": "explicitly authorized, permissioned, within policy",
        "neg_label": "unauthorized, bypassing controls, outside granted permissions",
    },
    "threat_severity": {
        "description": "Whether a text describes a high-severity, critical, or immediately dangerous threat vs a low-severity, routine, or benign concern",
        "pos_label": "high-severity, critical, immediately dangerous",
        "neg_label": "low-severity, routine, benign, non-critical",
    },
    "urgency": {
        "description": "Whether a text conveys time pressure, immediate action required, or crisis framing vs routine, low-priority, no time constraint",
        "pos_label": "urgent, time-critical, immediate action required",
        "neg_label": "routine, low-priority, no time pressure",
    },
    "exfiltration": {
        "description": "Whether a text describes or implies covert, unauthorized transfer of data out of a system vs legitimate, authorized data access or movement",
        "pos_label": "covert or unauthorized data exfiltration, data leaving a system without authorization",
        "neg_label": "legitimate, authorized data access or transfer within policy",
    },
    "obfuscation": {
        "description": "Whether a text deliberately hides intent, uses evasive or encoded language to avoid detection, or conceals meaning vs direct, transparent communication about the same subject",
        "pos_label": "evasive, encoded, deliberately obscuring intent or meaning",
        "neg_label": "direct, transparent, clearly stating intent and meaning",
    },
}
...
TOPICS = {
    ...
    "authorization": [
        "API key and token access control",
        ...
        "cross-tenant data access in SaaS",
        "service account credential use",
        "firewall rule change requests",
        "certificate and signing authority",
        "third-party vendor access provisioning",
        "emergency break-glass access procedures",
        "guest network access policies",
        "CI/CD pipeline secret access",
        "cloud IAM policy evaluation",
        "zero-trust verification for remote workers",
        "multi-factor authentication bypass requests",
        "data room access for M&A due diligence",
        "regulated data access for compliance audits",
        "shadow IT and unsanctioned tool use",
    ],
    "threat_severity": [
        "ransomware attack on hospital systems",
        "SQL injection in a public API",
        "phishing email targeting executives",
        "insider threat data access anomaly",
        "expired TLS certificate on internal service",
        ...
        "nation-state APT lateral movement",
        "supply chain compromise in open-source dependency",
        "failed login attempt from known IP",
        "unpatched OS on isolated development machine",
        "weak password policy enforcement",
        "shadow IT SaaS account without SSO",
        "malware beacon to command-and-control",
        "privilege escalation in production environment",
        "data breach notification from third-party vendor",
        "port scan detected on perimeter",
        "employee clicking phishing link on corporate device",
    ],
    "urgency": [
        "active ransomware encryption in progress",
        "scheduled quarterly security review",
        "imminent data breach window closing",
        "routine log rotation maintenance",
        "CEO fraud wire transfer request",
        "annual penetration test planning",
        "live DDoS attack on payment gateway",
        "low-priority software patch backlog",
        "expiring SSL certificate with 90 days remaining",
        "critical CVE with active exploitation in the wild",
        "monthly threat report review",
        "real-time insider threat alert",
        "deferred firewall rule cleanup",
        "production system credential compromise",
        "compliance audit scheduled for next quarter",
        "incident response activation at 2am",
        "routine security awareness training reminder",
        "live data exfiltration in progress",
        "end-of-life software migration planning",
        "emergency access revocation for terminated employee",
    ],
    "exfiltration": [
        "DNS tunneling to exfiltrate query logs",
        "authorized S3 bucket sync for backup",
        "steganographic encoding in image uploads",
        "routine ETL pipeline to data warehouse",
        "SMTP-based data extraction via email attachments",
        "approved API export for third-party analytics",
        "HTTP POST of encoded credentials to external endpoint",
        "scheduled database replication to DR site",
        "clipboard data capture by browser extension",
        "authorized SOC tool pulling endpoint telemetry",
        "covert FTP transfer of customer PII",
        "legitimate SaaS integration syncing CRM data",
        "USB mass storage write of classified documents",
        "approved removable media policy for air-gapped systems",
        "slow-and-low data bleed via outbound HTTPS",
        "regulated data export under GDPR data portability",
        "memory scraping of credential store",
        "IT-approved VPN split tunneling configuration",
        "compressed archive upload to personal cloud storage",
        "automated compliance report generation and distribution",
    ],
    "obfuscation": [
        "base64-encoded payload in HTTP header",
        "plain-text error message in application log",
        "polymorphic shellcode evading signature detection",
        "straightforward SQL query in audit trail",
        "junk instruction insertion in compiled binary",
        "clear comments in security policy document",
        "homoglyph substitution in domain name",
        "explicit data retention policy statement",
        "LOLBin execution via legitimate system tool",
        "direct API call with documented parameters",
        "typosquatted package name in dependency",
        "named and versioned open-source library import",
        "steganographic payload hidden in media file",
        "plaintext configuration file with documented fields",
        "double-encoded URL to bypass WAF",
        "standard URL encoding in REST API call",
        "process hollowing to masquerade as legitimate binary",
        "signed and verified system binary execution",
        "hex-encoded command string passed via environment variable",
        "human-readable shell script with inline comments",
    ],
}
